...
# Import data
```{r}
load("outputs/r/2_setup/bh-pb.RData") # 418 taxa and 131 samples
...
c100_pb <- c100 %>% 
  ...
  ungroup() # 3798 assignments

unique_pb <- c100_pb %>% select(db_pb) %>% unique() # 418 pb ASVs
...
sum(unique_pb_type$n) # 418

# How many amplicon ASVs per Pb ASV
c100_99 <- c100_pb %>% filter(id > 99) %>% #filter(type == ">99") %>% #
  group_by(db_pb) %>% summarize(count = n()) # 378
%>% write_csv("data/itol/bh_pb/bh_pb-asgn.csv")
...
```{r}
# Fig1 <- grid::rasterGrob(png::readPNG("outputs/Fig_1/pb_tree.png"), interpolate = T)
