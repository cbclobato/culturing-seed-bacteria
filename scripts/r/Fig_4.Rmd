...
# SI
```{r}
FigS7 <- cowplot::plot_grid(close, aspl,
                            neig, rad,
                            labels = c("A", "B", "C", "D"),
                            label_size = 12,
                            # vjust = 2,
...
node.data4 <- node.data3 %>% scale()
data.pca <- princomp(node.data4)
summary(data.pca)
...
set.seed(111)
ind <- sample(2, nrow(node.data3),
              replace = TRUE,
              prob = c(0.8, 0.2))
training <- node.data3[ind==1,]
testing <- node.data3[ind==2,]

pairs.panels(training[,-16],
             gap = 0,
             bg = c("red", "yellow", "blue")[training$recovered2],
             pch=21)

pc <- prcomp(training[,-16],
             center = TRUE,
            scale. = TRUE)
attributes(pc)
pc$center
...
g <- ggbiplot(pc,
              obs.scale = 1,
              var.scale = 1,
              groups = training$recovered2,
              ellipse = TRUE,
              circle = TRUE,
              ellipse.prob = 0.68)
g <- g + 
  scale_color_manual(name = "Fraction",
                       values = c("#0073C2FF", "#CD534CFF")) +
  theme_classic() +
  theme(legend.direction = 'vertical',
        legend.position = c(0.8, 0.9))
print(g)

cowplot::save_plot(plot = g,
  "outputs/r/SI/PCA.png",
  base_width = 20,
  base_height = 10,
  units = "cm",
  bg =  "white",
...
pc$loadings[, 1:2]
ev <- fviz_eig(pc, addlabels = TRUE); ev # Percentage of explained variances
fviz_pca_var(pc, col.var = "black")
vc <- fviz_cos2(pc, choice = "var", axes = 1); vc # How much each variable is represented in a given component.
fviz_pca_var(pc, col.var = "cos2",
            gradient.cols = c("black", "orange", "green"),
            repel = TRUE)

# cowplot::save_plot(plot = ev,
#   "outputs/r/SI/explained-var.png",
#   base_width = 20,
#   base_height = 10,
#   units = "cm",
#   bg =  "white",
