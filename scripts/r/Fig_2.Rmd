...
df1 <- df %>% mutate(type = factor(type, levels = c("(99, 100]", "(96.55, 99]", "(92.9, 96.55]", "(90.1, 92.9]",                                          "(87.95, 90.1]", "(85.93, 87.95]", "Uncultured"))) # "[77.35, 85.93]"
...
sorting.tax <- ordered(mer.strat$type,
                       levels = c("(99, 100]", "(96.55, 99]", "(92.9, 96.55]", "(90.1, 92.9]",                                          "(87.95, 90.1]", "(85.93, 87.95]", "Uncultured")) # "[77.35, 85.93]"
...
gen <- c100_strat  %>% filter(type == "(96.55, 99]") # 337 ASVs
fam <- c100_strat  %>% filter(type == "(92.9, 96.55]") # 501 ASVs
ord <- c100_strat  %>% filter(type == "(90.1, 92.9]") # 495 ASVs
cla <- c100_strat  %>% filter(type == "(87.95, 90.1]") # 309 ASVs
phy <- c100_strat  %>% filter(type == "(85.93, 87.95]") # 162 ASVs
...
data.g <- rbind(spe.ngen.c, tgen.c-spe.ngen.c) %>% mutate(Type = c("Cultured", "Uncultured"))
...
color.levels <- c("#dbdddd","#b89b74","#d9af63","#fdbf6f","#fdac4f","#ed8f47","#BF592A") # "#c7b699", 
levels <- rev(c("(99, 100]", "(96.55, 99]", "(92.9, 96.55]", "(90.1, 92.9]", "(87.95, 90.1]", "(85.93, 87.95]", "Uncultured")) #"[77.35, 85.93]"
```

### Stats
```{r}
# Is the RA of cultured taxa significantly different from the RA of the uncultured?
test <- df %>% filter(type == "(85.93, 87.95]" | type == "Uncultured")
...
Fig2C <- ggplot(df, aes(x = RA, fill = fct_relevel(type, levels))) +
  ...
  scale_fill_manual(values = c("(99, 100]" = "#BF592A",
                               ...
                               # "[77.35, 85.93]" = "#c7b699",
                               "Uncultured" = "#dbdddd")) +
  labs(x = "Mean relative abundance [%]", fill = "Similarity bins", color = "") +
