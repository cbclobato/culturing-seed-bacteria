...
sel.taxo <- sel.taxo %>% 
  mutate(type = factor(type, levels = rev(c("(99, 100]", "(96.55, 99]", "(92.9, 96.55]", "(90.1, 92.9]",                                          "(87.95, 90.1]", "(85.93, 87.95]","Uncultured"))), #"[77.35, 85.93]"
...
tile.raclass <- ggplot(sel.taxo,
  ...
  geom_text(aes(label = ifelse(RA_per >= 0.1, paste0(round(RA_per, 1), "%"), paste0(""))),
            color = "black", size = 2) +
  coord_fixed() +
  labs(x = "Class", y = "Similarity bins") +
  scale_x_discrete(expand = c(0, 0),
...
ra.genus.l <- ggplot(sel.taxo,
  ...
  scale_fill_manual(values = color.levels) +
  labs(x = "", y = "Cumulative mean relative abundance [%]", 
       fill = "Similarity bins") ; ra.genus.l
```
...
tile.ragenus <- ggplot(sel.taxo,
  ...
  geom_text(aes(label = ifelse(RA_per >= 0.1, paste0(round(RA_per, 1), "%"), paste0(""))),
            color = "black", size = 2) +
  coord_fixed() +
  labs(x = "Genus", y = "Similarity bins") +
  scale_x_discrete(expand = c(0, 0),
...
sel.taxo <- sel.taxo %>% 
  mutate(Class = factor(Class, levels = asv.n.taxo.total$Class),
         type = factor(type, levels = rev(c("(99, 100]", "(96.55, 99]", "(92.9, 96.55]", "(90.1, 92.9]",                                          "(87.95, 90.1]", "(85.93, 87.95]", "Uncultured")))) %>% #"[77.35, 85.93]",
  arrange(type)
